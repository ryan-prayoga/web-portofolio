  ...
  <div class="mt-12 grid gap-6 sm:grid-cols-2">
    {#each businessShowcases as work (work.slug)}
      <div
        ...
        class="bg-paper p-6 sm:p-8 flex flex-col justify-between transition-all hover:-translate-y-1 border border-rule"
      >
        <div>
          <div class="flex items-center justify-between gap-2 mb-3">
            <span class="font-mono text-[11px] uppercase tracking-wider text-accent font-semibold">
              {work.category[locale]}
            </span>
            <span class="font-mono text-[10px] text-muted bg-rule/30 px-2 py-0.5">
              {work.metrics}
            </span>
          </div>
