...
<section id="process" class="mx-auto max-w-5xl px-6 py-16 sm:py-24 border-t border-rule">
    ...
    {#each t.process.steps as step (step.step)}
      ...
      >
        <div>
          ...
          <h3 class="mt-3 font-display text-lg font-semibold text-ink tracking-tight">
            {step.title}
          </h3>
        ...
        <div class="mt-6 pt-4 border-t border-rule font-mono text-[11px] text-accent font-medium">
          <span class="text-muted block text-[10px] uppercase tracking-wider">Deliverable:</span>
          {step.output}
        </div>
      </div>
    {/each}
